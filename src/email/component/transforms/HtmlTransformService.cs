...
namespace legallead.email.transforms
{
    public class HtmlTransformService
    {
        ...
        public async Task<string?> GetHtmlTemplate(UserSettingQuery user, string templateName)
        {
            ...
            if (settingBo == null || settingBo.Count == 0) return TransformHtml(fallbackName);
            fallbackName = ExtractUserName(settingBo, fallbackName);
            return TransformHtml(fallbackName);

        }

        private string TransformHtml(string userName)
        {
            var html = new StringBuilder(BaseHtml);
            html.Replace(BodyHtmlToken, Substitutions[BodyHtmlToken] ?? string.Empty);
