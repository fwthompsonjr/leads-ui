...
{
    [TargetTable(TableName = "QUEUEWORKING")]
    public class QueueWorkingDto : BaseDto
    {
        ...
        public override object? this[string field]
        {
            ...
            set
            {
                ...
                {
                    SearchId = ChangeType<string>(value) ?? string.Empty;
                    return;
                }
                if (fieldName.Equals("Message", Comparison))
                {
                    Message = ChangeType<string>(value) ?? string.Empty;
                    return;
                }
                if (fieldName.Equals("MachineName", Comparison))
                {
                    MachineName = ChangeType<string>(value) ?? string.Empty;
                    return;
                }
                if (fieldName.Equals("StatusId", Comparison)) { StatusId = ChangeType<int?>(value); }
                if (fieldName.Equals("CreateDate", Comparison)) CreateDate = ChangeType<DateTime?>(value);
                if (fieldName.Equals("LastUpdateDt", Comparison)) LastUpdateDt = ChangeType<DateTime?>(value);
