...
{
    public class UserPermissionViewRepositoryTests
    {
        ...
        {
            UserPermissionView[] result = Array.Empty<UserPermissionView>();

            var container = new RepoContainer();
            var service = container.Repo;
            var mock = container.CommandMock;
            mock.Setup(m => m.QueryAsync<UserPermissionView>(
                It.IsAny<IDbConnection>(),
                It.IsAny<string>(),
                It.IsAny<DynamicParameters>()
            )).ReturnsAsync(result);
            await service.GetAll(new User());
            mock.Verify(m => m.QueryAsync<UserPermissionView>(
                It.IsAny<IDbConnection>(),
                It.IsAny<string>(),
                It.IsAny<DynamicParameters>()
            ));
        ...
        {
            var result = new UserPermissionView();
            var container = new RepoContainer();
            var service = container.Repo;
            var mock = container.CommandMock;
            mock.Setup(m => m.QuerySingleOrDefaultAsync<UserPermissionView>(
                It.IsAny<IDbConnection>(),
                It.IsAny<string>(),
                It.IsAny<DynamicParameters>()
            )).ReturnsAsync(result);
            await service.DoesRecordExist(new User(), faker.Random.AlphaNumeric(12));
            mock.Verify(m => m.QuerySingleOrDefaultAsync<UserPermissionView>(
                It.IsAny<IDbConnection>(),
                It.IsAny<string>(),
                It.IsAny<DynamicParameters>()
            ));
        ...
        {
            private readonly UserPermissionViewRepository repo;
            ...
            public RepoContainer()
            {
                ...
                repo = new UserPermissionViewRepository(dataContext);
            }

            public UserPermissionViewRepository Repo => repo;
